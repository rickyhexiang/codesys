 ...
 *  </copyright>
 */
#ifndef _CMPBLDDEP_H_
#define _CMPBLDDEP_H_

#define COMPONENT_NAME "CmpBLD"COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpBLD)
#define COMPONENT_NAME_UNQUOTED CmpBLD
...
#define CMPID_CmpBLD		0x2006								/* NOTE: START HERE WITH YOUR COMPONENTIDS (see CmpItf.h */
#define CLASSID_CCmpBLD	    ADDVENDORID(CMP_VENDORID, 0x2006)	/* NOTE: START HERE WITH YOUR CLASSIDS (see CmpItf.h */
#define ITFID_ICmpBLD		ADDVENDORID(CMP_VENDORID, 0x2006)	/* NOTE: START HERE WITH YOUR INTERFACEIDS (see CmpItf.h */
...
#ifndef CMPBLD_DISABLE_EXTREF
#define EXPORT_EXTREF_STMT \
        { (RTS_VOID_FCTPTR)bld__main, "bld__main", 0x66125D34, 0x01000000 },\
          
#else
#define EXPORT_EXTREF_STMT
#endif
#ifndef CMPBLD_DISABLE_EXTREF2
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpBLD__Entry
#endif


#ifdef CPLUSPLUS

class CCmpBLD : public ICmpBLD 
{
    public:
        CCmpBLD() : hCmpBLD(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpBLD()
        {
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((ICmpBLD *)this);            
            else if (iid == ITFID_ICmpBLD)
                pItf = dynamic_cast<ICmpBLD *>(this); 
            else
            {
                if (pResult != NULL)
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpBLD_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpBLD_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
