 ...
 *  </copyright>
 */
#ifndef _CMPENCODERCTRLDEP_H_
#define _CMPENCODERCTRLDEP_H_

#define COMPONENT_NAME "CmpEncoderCtrl"COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpEncoderCtrl)
#define COMPONENT_NAME_UNQUOTED CmpEncoderCtrl
...
#ifndef CMPENCODERCTRL_DISABLE_EXTREF
#define EXPORT_EXTREF_STMT \
        { (RTS_VOID_FCTPTR)setencodercounter__main, "setencodercounter__main", 0x7E81583C, 0x01000000 },\
          { (RTS_VOID_FCTPTR)readencoderdirection__main, "readencoderdirection__main", 0x6DFB027B, 0x01000000 },\
          { (RTS_VOID_FCTPTR)readencodercounterz__main, "readencodercounterz__main", 0x3A4B34EB, 0x01000000 },\
          { (RTS_VOID_FCTPTR)readencodercountera__main, "readencodercountera__main", 0x9DA91CE6, 0x01000000 },\
          { (RTS_VOID_FCTPTR)configencoder__main, "configencoder__main", 0x06280A78, 0x01000000 },\
          
#else
#define EXPORT_EXTREF_STMT
#endif
#ifndef CMPENCODERCTRL_DISABLE_EXTREF2
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpEncoderCtrl__Entry
#endif


#ifdef CPLUSPLUS

class CCmpEncoderCtrl : public ICmpEncoderCtrl 
{
    public:
        CCmpEncoderCtrl() : hCmpEncoderCtrl(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpEncoderCtrl()
        {
        }
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((ICmpEncoderCtrl *)this);            
            else if (iid == ITFID_ICmpEncoderCtrl)
                pItf = dynamic_cast<ICmpEncoderCtrl *>(this); 
            else
            {
                if (pResult != NULL)
    ...
    public:
        RTS_HANDLE hCmpEncoderCtrl;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpEncoderCtrl Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpEncoderCtrl_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpEncoderCtrl_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
