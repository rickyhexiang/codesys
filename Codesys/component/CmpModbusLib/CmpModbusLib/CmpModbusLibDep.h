 ...
 *  </copyright>
 */
#ifndef _CMPMODBUSLIBDEP_H_
#define _CMPMODBUSLIBDEP_H_

#define COMPONENT_NAME "CmpModbusLib"COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpModbusLib)
#define COMPONENT_NAME_UNQUOTED CmpModbusLib
...
#ifndef CMPMODBUSLIB_DISABLE_EXTREF
#define EXPORT_EXTREF_STMT \
        { (RTS_VOID_FCTPTR)modbuswritereg__main, "modbuswritereg__main", 0x430FBCF6, 0x01000000 },\
          { (RTS_VOID_FCTPTR)modbuswritebit__main, "modbuswritebit__main", 0x65549537, 0x01000000 },\
          { (RTS_VOID_FCTPTR)modbusslavewritereg__main, "modbusslavewritereg__main", 0x9CB5E23D, 0x01000000 },\
          { (RTS_VOID_FCTPTR)modbusslavewritebit__main, "modbusslavewritebit__main", 0xB46A3E05, 0x01000000 },\
          { (RTS_VOID_FCTPTR)modbusslavereadreg__main, "modbusslavereadreg__main", 0x0A3164DE, 0x01000000 },\
          { (RTS_VOID_FCTPTR)modbusslavereadbit__main, "modbusslavereadbit__main", 0x9E88DCD4, 0x01000000 },\
          { (RTS_VOID_FCTPTR)modbusreadreg__main, "modbusreadreg__main", 0x3CF79B14, 0x01000000 },\
          { (RTS_VOID_FCTPTR)modbusreadbit__main, "modbusreadbit__main", 0x71236F41, 0x01000000 },\
          { (RTS_VOID_FCTPTR)initmodbusrtu__main, "initmodbusrtu__main", 0x28BA41FA, 0x01000000 },\
          { (RTS_VOID_FCTPTR)closemodbus__main, "closemodbus__main", 0x32BCE1D3, 0x01000000 },\
          
#else
#define EXPORT_EXTREF_STMT
#endif
#ifndef CMPMODBUSLIB_DISABLE_EXTREF2
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpModbusLib__Entry
#endif


#ifdef CPLUSPLUS

class CCmpModbusLib : public ICmpModbusLib 
{
    public:
        CCmpModbusLib() : hCmpModbusLib(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpModbusLib()
        {
        }
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((ICmpModbusLib *)this);            
            else if (iid == ITFID_ICmpModbusLib)
                pItf = dynamic_cast<ICmpModbusLib *>(this); 
            else
            {
                if (pResult != NULL)
    ...
    public:
        RTS_HANDLE hCmpModbusLib;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpModbusLib Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpModbusLib_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpModbusLib_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
