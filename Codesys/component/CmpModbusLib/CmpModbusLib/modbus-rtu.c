...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    ctx->s = open(ctx_rtu->device, flags, 0);
    if (ctx->s == -1) {
        if (ctx->debug) {
    ...
    */
    ...
    ioctl(ctx->s, FIOBAUDRATE, ctx_rtu->baud);   /* ²¨ÌØÂÊ */
	/* debug */
	printf("baud=%d\n", ctx_rtu->baud);
...
#endif
	ioctl(ctx->s, SIO_HW_OPTS_SET, tios.c_cflag);
	/* debug */
	printf("opts=%x\n", tios.c_cflag);
