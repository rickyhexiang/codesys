#ifndef _GLODON_IOT_ERROR_H_
#define _GLODON_IOT_ERROR_H_
 ...
 */
typedef enum {
	/** Returned when the payload is treated. Nothing to do */
			PAYLOAD_PRETREATMENT_FINISH = 7,
	/** Returned when the IoTNetwork physical layer is connected */
			NETWORK_PHYSICAL_LAYER_CONNECTED = 6,
	/** Returned when the IoTNetwork is manually disconnected */
			NETWORK_MANUALLY_DISCONNECTED = 5,
	/** Returned when the IoTNetwork is disconnected and the reconnect attempt is in progress */
	...
	/** Returned when the IoTNetwork is disconnected and reconnect is either disabled or physical layer is disconnected */
			NETWORK_DISCONNECTED_ERROR = -13,
	/** Returned when the IoTNetwork is disconnected and the reconnect attempt has timed out */
			NETWORK_RECONNECT_TIMED_OUT_ERROR = -14,
	/** Returned when the IoTNetwork is already connected and a connection attempt is made */
			NETWORK_ALREADY_CONNECTED_ERROR = -15,
	/** IoTNetwork layer Error Codes */
	/** IoTNetwork layer Random number generator seeding failed */
			NETWORK_MBEDTLS_ERR_CTR_DRBG_ENTROPY_SOURCE_FAILED = -16,
	/** A generic error code for IoTNetwork layer errors */
	...
	/** An error occurred while parsing the JSON string.  Usually malformed JSON. */
			JSON_PARSE_ERROR = -42,
	/** An error occurred while add JSON value.  Usually additerm JSON. */
			JSON_ADDITERM_ERROR = -43,
	...
	/** Returned when the IoTNetwork is DNS error */
			NETWORK_DNS_ERROR = -50,
} IoTError;
